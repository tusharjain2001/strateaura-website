...
/**
 * Request Brochure form popup — Figma node 1816:734.
 *
 * Opened from the VEIL™ section's "Request Brochure" button. On submit it POSTs
 * to /api/brochure, which emails the team (training@strateaura.com) the request
 * and sends the visitor a confirmation. `program` labels which brochure was
 * asked for so the team email is specific.
 */
export default function RequestBrochureModal({ onClose, program = "" }) {
  ...
  async function handleSubmit(event) {
    ...
    setError("");
    try {
      await postJson("/api/brochure", { ...fields, program });
      setDone(true);
    } catch (err) {
        ...
        {done ? (
          <div className="flex flex-col gap-3">
            ...
            <p className="text-[16px] leading-normal font-light text-navy-2">
              Your request has been received. We&apos;ve emailed your brochure to you — it should
